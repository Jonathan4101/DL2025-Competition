...
def inference(args, model, test_save_path=None):
    ...
    for image_idx in range(len(image_fns)):
        ...
        for i, (data, coords) in enumerate(dataloader):
            ...
            with torch.no_grad():
            # 原图
                out1_1, out2_1 = model(data)
                prob1 = F.softmax((out1_1 + out2_1)/2, dim=1)

                # 水平翻转
                data_h = torch.flip(data, dims=[3])
                out1_2, out2_2 = model(data_h)
                prob2 = F.softmax((out1_2 + out2_2)/2, dim=1)
                prob2 = torch.flip(prob2, dims=[3])  # 翻回来

                # 垂直翻转
                data_v = torch.flip(data, dims=[2])
                out1_3, out2_3 = model(data_v)
                prob3 = F.softmax((out1_3 + out2_3)/2, dim=1)
                prob3 = torch.flip(prob3, dims=[2])

                # 三个结果平均
                t_output = ((prob1 + prob2 + prob3) / 3.0).cpu().numpy()
